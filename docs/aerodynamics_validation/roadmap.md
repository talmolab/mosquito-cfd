...
| ✅ **T3c** ([#50](https://github.com/talmolab/mosquito-cfd/issues/50), PR [#52](https://github.com/talmolab/mosquito-cfd/pull/52)) | **Fine-grid 256×128×256 run + 3-grid Richardson study** — ran on local RTX A5000 (dt=0.00025 D6 fallback, 4000 steps, ~8.8 h); `iamrex_commit=f93dc794`. 3-grid Richardson: **CF_normal monotone** (p_obs=1.38, Richardson=2.162, GCI_fine=3.7%); **CF_chord non-monotone** (0.923→0.554→0.961 — IB-coupling caveat prevents monotone convergence at 256³; Richardson N/A). Further refinement (512³+) deferred to H100/grant. | **DONE.** `forces_fine.csv` + `run_metadata_t3c.json` committed; RESULTS.md T3c section + reproducibility guard added; #50 closed. | Yes (local A5000) | **DONE** |
