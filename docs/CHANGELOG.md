...
- Committed synthetic-fixture-derived reference figures `docs/visualization/coarse_vs_fine_comparison.png` and `docs/visualization/diagnostic_config_mean_collapse.png` (with a `docs/visualization/README.md` disclosing they are fixture-derived, not real corpus data, plus regeneration fixtures/commands); `openspec/project.md` "Visualization Tooling" section and a Dependencies-list fix noting the `train`/`viz` optional groups (#75)
