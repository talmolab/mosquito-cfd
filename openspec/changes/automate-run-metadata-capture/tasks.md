...
- [ ] 0.1 Add `tests/fixtures/run_metadata/` with, for the `s35_f085_p45` pilot config: a copy of
  ...
  json`-shaped status response with `startedAt`/`finishedAt` timestamps consistent with the
  pilot's reported wall time (7032.46s / 1h58m).
...
- [ ] 0.3 **Fix the `.gitignore` gap (D6):** generalize `examples/prelim_sweep/runs/` (line 24) to
...
- [ ] 0.4 **Test first (fixture drift guard):**
...
- [ ] 1.0 Create `src/mosquito_cfd/force_surrogate/metadata_capture.py` as the home for all logic
...
- [ ] 2.1 **Test first:** `tests/test_metadata_capture.py::test_read_final_time_from_csv_uses_last_row`
  — given the fixture CSV, assert the reader returns `time=2.3525` and `timesteps=4706`, not the
  deck's `stop_time=2.352941176`.
- [ ] 2.2 **Test first:** `test_read_final_time_raises_on_empty_csv` — a CSV with header only
  raises a clear error, not an `IndexError`.
- [ ] 2.3 **Test first:** `test_read_final_time_raises_on_missing_csv_file` — a nonexistent CSV
  path raises a clear, path-naming error.
- [ ] 2.4 Implement the reader in `metadata_capture.py` to make 2.1–2.3 pass.

## 3. run.log Arena-max parser and stability derivation (TDD, cluster-free)

- [ ] 3.1 **Test first:** `test_parse_arena_max_mib_from_run_log` — given the fixture `run.log`
  excerpt, assert the parser extracts `7998` (or the fixture's chosen value) as an integer MiB
  figure.
- [ ] 3.2 **Test first:** `test_parse_arena_max_mib_returns_none_when_absent` — a `run.log` with no
  Arena line returns `None` rather than raising.
- [ ] 3.3 **Test first:** `test_parse_arena_max_mib_raises_on_missing_log_file` — a nonexistent
  `run.log` path raises a clear, path-naming error (distinct from "no Arena line present").
- [ ] 3.4 **Test first (D2's concrete stability rule):**
...
- [ ] 3.5 Implement the parser and the `derive_stability(fixed_dt)` function in
  `metadata_capture.py` to make 3.1–3.4 pass.

## 4. Manifest/deck sourcing (TDD, cluster-free)

- [ ] 4.1 **Test first:** `test_kinematics_grid_fixed_dt_max_step_sourced_from_manifest` — given
  ...
  kinematics).
- [ ] 4.2 **Test first:** `test_manifest_lookup_raises_on_missing_config` — an unknown config name
  raises a clear `KeyError`-wrapping error naming the config and the manifest path.
- [ ] 4.3 **Test first:** `test_manifest_lookup_raises_on_malformed_manifest_json` — a manifest
...
- [ ] 4.4 Implement to make 4.1–4.3 pass.

## 5. Docker-digest and git-commit passthrough validation (TDD, cluster-free)

- [ ] 5.1 **Test first:** `test_docker_digest_field_is_single_named_and_validated` — given a
...
- [ ] 5.2 **Test first:** `test_malformed_digest_is_rejected` — given a pod-side `run_metadata.json`
  ...
  offending value.
- [ ] 5.3 **Test first:** `test_git_commit_must_be_full_sha` — given a pod-side `run_metadata.json`
...
- [ ] 5.4 **Test first:** `test_pod_run_metadata_raises_on_missing_file` — a nonexistent pod-side
  `run_metadata.json` path raises a clear, path-identified error.
- [ ] 5.5 **Test first:** `test_pod_run_metadata_raises_on_malformed_json` — a pod-side
  `run_metadata.json` that fails to parse as JSON raises a clear, path-identified error (distinct
  from 5.4's "file doesn't exist").
- [ ] 5.6 Implement to make 5.1–5.5 pass.

## 6. Argo workflow-status query wrapper (TDD, injected/faked)

- [ ] 6.1 **Test first:** `test_wall_time_from_argo_status_timestamps` — given the fixture Argo
...
- [ ] 6.2 **Test first:** `test_wall_time_reflects_only_final_successful_attempt` — given a fixture
...
- [ ] 6.3 **Test first:** `test_wall_time_s_override_bypasses_argo_query` — given `--wall-time-s
  7032.46` is supplied, assert the injected Argo-query function is never called and `wall_time_s`
  is `7032.46` exactly.
- [ ] 6.4 **Test first:** `test_argo_status_missing_timestamps_raises_clear_error` — a status
...
- [ ] 6.5 **Test first:** `test_argo_status_query_failure_produces_clear_error` — a query that
...
- [ ] 6.6 Implement the wrapper as an injectable function in `metadata_capture.py` (default
...
- [ ] 7.1 **Test first:** `test_assemble_metadata_produces_normalized_schema` — given all fixture
...
- [ ] 7.2 **Test first:** `test_assemble_metadata_notes_field_optional` — omitting `--notes`
  produces valid output with no `notes` key; passing `--notes "..."` includes it verbatim.
- [ ] 7.3 **Test first:** `test_assemble_metadata_raises_on_row_count_mismatch_between_pod_and_csv`
...
- [ ] 7.4 **Test first:** `test_assemble_metadata_matches_known_correct_pilot_config` — the
...
- [ ] 7.5 Implement the assembler in `metadata_capture.py` to make 7.1–7.4 pass. The module's
...
- [ ] 8.1 **Test first:** `test_cli_writes_output_file` — invoke `scripts/generate_run_metadata.py`
...
- [ ] 8.2 **Test first:** `test_cli_rejects_missing_required_args` — omitting `--tier` or
  `--workflow-name` fails with a clear argparse error, not a downstream `KeyError`.
- [ ] 8.3 **Test first:** `test_cli_wall_time_s_flag_skips_argo_query` — passing `--wall-time-s`
  produces output without invoking the Argo-query path.
- [ ] 8.4 Implement `scripts/generate_run_metadata.py`'s `main()`/argparse wiring (thin wrapper
...
- [ ] 9.2 Annotate `docs/force_surrogate/fine-grid-pilot-report.md`'s "schema matches
...
- [ ] 9.3 Correct `docs/force_surrogate/roadmap.md` CC-1's run-metadata field description: it
...
- [ ] 9.4 Add a short usage note (module docstring in `scripts/generate_run_metadata.py`) pointing
...
- [ ] 11.1 `openspec validate automate-run-metadata-capture --strict`.
- [ ] 11.2 `uv run pytest` (full suite green).
- [ ] 11.3 `uv run ruff check src/ scripts/ tests/` (a subset of CI's full lint scope, which also
...
- [ ] 11.4 `uv run ruff format --check src/ scripts/ tests/`.
