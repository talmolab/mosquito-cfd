...
| **T2a** 🟡 | **Axis-convention + MOTION refactor (issue #1) — solver + convention + doc, atomic** — `wing.vertex` span→y, deck BC change **`z` wall→outflow** (y-periodic retained = the span; NOT "z wall→periodic"), `WingKinematics.H` Euler `Rz(φ)·Ry(α)·Rx(θ)`, hinge coords, **`docs/coordinate-convention.md` + the `WingKinematics.H` docstring + the RESULTS frame-description in the SAME change.** | **Decoupled (CC-V4):** (1) analysis-layer **rotation-equivariance** guard on the generalized field extractor (cluster-free); (2) **body-frame** per-component van Veen comparison (`CF_chord`/`CF_normal` vs fitted `C_F`, `[0.5,1.5]` floor) on the new-convention re-run; (3) **stroke-motion** verification (span-tip sweeps). The refactor is a **motion change** (forces differ old↔new), so sim-to-sim "invariance" is NOT the oracle. No interim commit carries new-convention geometry with old-convention docs. **Cluster-free core landed; the coarse A40 re-run + body-frame force numbers pending.** | Yes (coarse) | **post-submission** |
