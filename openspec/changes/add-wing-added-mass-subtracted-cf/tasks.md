...
1.1 [ ] **Test first (unit / synthetic — linearity + pure-chord cancel):** in
    ...
    missing.
1.2 [ ] **Test first (reuse is real, not re-derived — T-3):** add
...
1.3 [ ] **Implement** `body_frame_added_mass_subtracted(csv_path, *, f_star, phi_amp_deg, pitch_amp_deg,
...
2.1 [ ] **Test first (committed CSV, pin `pitch_amp_deg=45.0`):** add
...
2.2 [ ] **Test first (RMS shares + definition guard — T-5):** add `test_added_mass_body_frame_rms_shares`
...
3.1 [ ] **Test first:** add `test_diagnostic_is_reported_not_graded` asserting (a) `set(out)` equals the
...
4.1 [ ] **Test first:** add `test_added_mass_subtracted_guards` — parametrized over dropping **each** of
    ...
    set in 1.3.
4.2 [ ] **Test first (`window_t0` + `rho_f` plumbing — T-6 / T-8):** add
    ...
    Passes once 1.3.
4.3 [ ] **Test first (peak-migration + signed drop_frac — physics edge cases):** add
...
5.1 [ ] **Test first (doc numbers guard):** in `tests/test_results_reproducibility.py`, add
    ...
    **asserted-complete via a set-equality scan** of that subsection (a new interim number absent from the
    ...
    doc is edited.
5.2 [ ] **Test first (honest-framing + disambiguation + same-peaks wording — T-1 / D-2 / D-3 / D-5):** add
...
5.3 [ ] **Edit `examples/flapping_wing/RESULTS.md`:** add subsection *"Added-mass-subtracted body-frame
    ...
    84 % RMS energy share). Add the interim numbers to the `### Body-frame per-component` table; give the
    unguarded `## Comparison with van Veen` table a **bare cross-reference** to the interim subsection (no
    re-transcribed number — it is outside the enumeration guard); update the Validation-Status row to
    reference the interim, verdict **PARTIAL** + `#40`. 5.1 + 5.2 now pass.

## 6. Traceability

6.1 [ ] **Edit `docs/aerodynamics_validation/roadmap.md`** (T4 / #40 row): one-line note that the cheap
...
7.1 [ ] `uv run ruff check .` and `uv run ruff format --check .` clean (run `ruff format` before **each**
    commit that touches `src/`/`tests/`, not only at the end — CI checks formatting per push).
7.2 [ ] `uv run pytest` — full suite green (new + existing; the existing reproducibility / not-loosened /
    enumeration guards still pass).
7.3 [ ] `"/c/Users/Elizabeth/AppData/Roaming/npm/openspec" validate add-wing-added-mass-subtracted-cf
    --strict` passes.

## 8. Commit & PR discipline (CI green at every pushed commit)

8.1 [ ] **Commit grouping** (3 atomic commits, each CI-green):
