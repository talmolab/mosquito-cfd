...
| Flapping wing — **validation** | van Veen's **quasi-steady model** (translational + added-mass + Wagner) replotted at our kinematics (**not** a digitized curve — the model is rebuilt from published coefficients) matches the CFD: normal **peak magnitude within tol** (graded); peak-phase + curve RMSE **reported**. (Fig 4 = the coefficient-vs-α polars we build the model from; Fig 13 = van Veen's time-resolved mosquito curves.) | **T4** | van Veen 2022 Fig 4 / Fig 13 |
...
| ✅ **T3a** ([#46](https://github.com/talmolab/mosquito-cfd/issues/46), PR [#47](https://github.com/talmolab/mosquito-cfd/pull/47)) | **Medium-grid convergence tooling + deck (cluster-free)** — re-author the new-convention 128×64×128 deck (`inputs.3d.convergence_medium`; **only `amr.n_cell` changes** vs the confirmed coarse `inputs.3d.validation`, the old `inputs.3d.production` is old-BC/different-operating-point/already-128³ and is NOT reused), and build + TDD a **report-only** 2-grid convergence grader (`benchmarks/wing_convergence.py`) + LEV vorticity/Q pure functions (`benchmarks/lev.py`) against the committed coarse run + synthetic fixtures. **Quantify** grid sensitivity; do not gate it. | **Tooling delivered + unit-tested cluster-free** — deck-invariance (differs from coarse **only** in `amr.n_cell`; `fixed_dt`/`radius` held), grader known-answer + report-only + degenerate/sign guards, LEV known-analytic vorticity (`\|ω\|=2Ω`, `Q=Ω²`). **No graded run here** (grading is T3b). | No (analysis-only) | **in-bounds** |
| ✅ **T3b** ([#46](https://github.com/talmolab/mosquito-cfd/issues/46)) | **Medium-grid run + grading (operator A40)** — ran the medium sim (`:fp64 @ f93dc794`, A40, `capture_run_metadata`), committed `forces_medium.csv` + `run_metadata_t3b.json`, applied the T3a grader + LEV to the real data, wrote the RESULTS convergence section. | **DONE.** Reported the coarse↔medium **relative change + 2-grid GCI band (p = 1..2)**: peak **CF_chord −66.5 %** (0.923→0.554, GCI band 0.28–0.83) — a large drop showing **coarse-grid under-resolution was a major contributor** to the #40 chord excess, though **not grid-converged at medium** (chord `gci_p1=0.83`; whether the residual gap to van Veen's ~0.3 is further grid error or physical is **undetermined from 2 grids** — van Veen's 0.3 is inside the GCI band under 1st-order, outside under 2nd); **CF_normal −11.7 %** (grid-settled). LEV present on **both** grids at mid-stroke (resolution-fair ∫Q⁺ +9 %; peak Q ×3 is a resolution artifact). Report-only, no pass bar. True observed-order (3-grid/256³) verdict deferred (H100/grant, [#50](https://github.com/talmolab/mosquito-cfd/issues/50)); **#40 advanced, not resolved** (T4 reuses these runs). | Yes (A40 unattended) | **in-bounds** |
| ✅ **T4** ([#40](https://github.com/talmolab/mosquito-cfd/issues/40), PR [#51](https://github.com/talmolab/mosquito-cfd/pull/51)) | **Per-component force validation + decomposition** — build van Veen's **quasi-steady model (translational + added-mass + Wagner)** from its published coefficients, apply it to our kinematics, and compare each component + the total to the CFD `ib_force` (the model is **replotted, not a digitized Fig 3–4**; Fig 4 = the coefficient-vs-α polars we build it from, Fig 13 = van Veen's time-resolved mosquito curves). Resolves the T2a CF_chord PARTIAL. This tier **validates the wing against van Veen's quasi-steady model** (in peak magnitude — a consistency result at matched kinematics, not an independent measurement of the per-component split); T2b/T3 only establish in-band plausibility. | **DONE.** Normal peak **magnitude** graded (model 2.48 vs CFD 2.61, relative gap ~5 % < 16 % tol; grid-dominated) + decomposition **closure**; the normal peak-**phase** gap ~0.058 cycle (the CFD *leads* the QS model — expected quasi-steady-vs-unsteady discrepancy, grid + single-wingbeat-transient confounded) and the curve RMSE are **reported, not gated**; the translational-chord known-answer ~0.42 ≪ the total 0.92 (the #40 apples-to-oranges); the CFD chord converges toward the model under refinement (0.92 → 0.554 → ~0.43, the tight grid verdict deferred to **[#50](https://github.com/talmolab/mosquito-cfd/issues/50)**). **#40 resolved.** | reuses T3 runs | **DONE** |
| ⬜ **T3c** ([#50](https://github.com/talmolab/mosquito-cfd/issues/50)) | **Fine-grid 256×128×256 run + 3-grid Richardson study (operator A40)** — runs the fine 256³ grid (same `:fp64 @ f93dc794` IAMReX pin, `amrex.the_arena_init_size=28` proactive arena cap), commits `forces_fine.csv` + `run_metadata_t3c.json`, and applies the 3-grid convergence grader (`wing_grid_convergence_from_body_forces(..., fine_csv=fine)`) to compute the **observed convergence order p** and an **illustrative Richardson extrapolant `cf_exact_richardson`** per component (IB coupling caveat: each delta reflects combined spatial + IB-model refinement; cf_exact_richardson is NOT a definitive h→0 limit). Report-only: "converging", "not yet converged", and "non-monotone" are all valid, informative outcomes; no pass bar. Session A (cluster-free): fine-grid deck + 3-grid tooling + TDD tests. Session B (post-run): data + RESULTS + reproducibility guard. | **Session A in progress.** Fine deck + `wing_grid_convergence_3grid` + `assert_gradeable_triple` + cluster-free tests delivered. Operator A40 fine run pending (see `docs/aerodynamics_validation/t3c-handoff.md`). | Yes (A40 unattended) | **in-bounds** |

**Sequencing.** T1a → T1b → T2a → T2b → T3a → T3b → T4 → **T3c**. **T1a…T4 complete (2026-07).**
T4 (per-component force decomposition, #40) **validated the wing against van Veen's quasi-steady model** (in
peak magnitude) and **resolved the CF_chord PARTIAL** — the total-chord had been compared apples-to-oranges
to van Veen's translational-only ~0.3; the decomposition shows the total = transl + added-mass + Wagner.
**T3c** ([#50](https://github.com/talmolab/mosquito-cfd/issues/50)) extends T3b to the **3-grid Richardson
study** — observed convergence order + illustrative `cf_exact_richardson` per component (IB coupling
caveat: the diffused-IB regularization sharpens with the grid, so `cf_exact_richardson` is an illustrative
estimate, not a definitive h→0 limit). The tight chord grid verdict (#50) is the last open aerodynamic-
validation item. T3b landed the graded medium run + the RESULTS "Grid convergence (T3b)" section
(CF_chord drops 66.5 % under refinement — not yet grid-converged). T1 was the only pre-deadline work;
with June 30 passed, T4 was in-bounds (post-submission window).
...
- **T3 EPIC (filed just-in-time):** **[#46](https://github.com/talmolab/mosquito-cfd/issues/46)**
  (medium-grid grid convergence; split **T3a** tooling+deck / **T3b** operator run + grading).
...
- **Fine grid (256³+) flapping beyond T3c** — the T3c 256×128×256 run (#50) is **in-flight**
  (Session A complete; operator A40 run pending). If T3c's observed order shows the chord is still
  not converged at fine, further refinement (512³+) defers to H100 (grant).
...
- **[BLOCKING] Flapping "CF in [0.5,1.5]" is a plausibility band, not validation; title overclaimed** →
  ...
  reserved "validated" for the sphere (T1b) and the wing (T4 — **validated against van Veen's
  quasi-steady model**, in peak magnitude).
...
### T3 oracle relaxation — graded → reported (2026-07-05, `add-wing-grid-convergence` / T3a, [#46](https://github.com/talmolab/mosquito-cfd/issues/46))

- **T3's original graded oracle ("CF converged *within tol*; LEV present") is relaxed to a *reported*
  diagnostic.** A rigorous "converged" verdict is not defensible here for **two** compounding reasons:
  (1) the observed order of convergence is **unobservable from 2 grids** (it needs a 3rd), and (2) the
  diffused IB is **grid-tied** — the marker volume `dv = h·d_nn²` and the regularization kernel support
  scale with the grid spacing `h`, so refining the grid also **sharpens the IB-regularization model**,
  a change Richardson extrapolation cannot model (no `cf_exact` limit for that part). So T3 now
  **reports** the coarse↔medium **relative change + a 2-grid GCI band** (orders p = 1..2, the
  diffused-IB order range — `gci_p1` is the reported band edge, **not** a rigorous upper bound) on peak
  body-frame CF_chord/CF_normal, and **reports** the LEV vorticity/Q — with **no pass bar, no loosenable
  tolerance constant, and no Richardson `cf_exact`**. "Not converged at coarse" is a valid, informative
  outcome (feeds #40). The **true observed-order (3-grid / 256³) convergence verdict is deferred to the
  H100/grant** (see *Out of scope*). The T3 row is **split T3a** (this change: cluster-free tooling +
  deck, TDD'd against the committed coarse run + synthetic fixtures) **/ T3b** (operator medium run +
  grading + RESULTS) — mirroring the T2a/T2b split, so T3 is not silently marked "done" on weaker
  evidence than its original row demanded.

### T4 normal peak-PHASE reported, not gated (2026-07-09, `decompose-wing-force-per-component` / T4, [#40](https://github.com/talmolab/mosquito-cfd/issues/40))

- **T4's normal peak-phase and curve RMSE are *reported*, not gated; the graded oracle rests on the
  normal peak *magnitude* + decomposition closure.** Measured on the committed data, the CFD normal peak
  **leads** van Veen's quasi-steady model by ~0.058 cycle — the expected quasi-steady-vs-unsteady
  discrepancy (the QS model omits the Wagner wake-memory / added-mass phase *history* the CFD resolves),
  and it is **triply confounded** (QS-intrinsic + grid non-convergence + the single-wingbeat transient). A
  physics-honest phase tolerance would be ~0.02 cycle, which the confounded gap exceeds; **widening a phase
  gate to fit the ~0.06 measured gap would be reverse-fitting** (CC-V2). So the phase gap is reported with
  its confounds. This is an **evidence-based scoping decision, NOT a loosened tolerance** — the one graded
  magnitude tolerance (`T4_NORMAL_MAG_TOL`) stays sourced (grid GCI ⊕ coefficient CI ⊕ S_WE uncertainty)
  and is recomputed from its inputs by a test. The magnitude claim is honestly hedged as **validated
  *against* van Veen's quasi-steady model** (a consistency result, grid-dominated), not a bare "validated".
