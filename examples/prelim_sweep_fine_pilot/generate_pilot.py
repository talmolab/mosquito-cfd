...
# reuse a stale timestamp) and now require --timestamp explicitly, validated via the shared
# mosquito_cfd.force_surrogate.sweep.iso8601_timestamp. This script keeps the default deliberately
# -- the pilot isn't re-run by that change -- but the same latent footgun exists here if this
# script is ever used for a real regeneration; make --timestamp required (type=iso8601_timestamp,
# reused from sweep.py, not re-implemented) then too.
