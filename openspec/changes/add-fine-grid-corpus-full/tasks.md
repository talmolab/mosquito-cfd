...
- [x] 1.1 **Test first (config count and grid defaults):** in a new `tests/test_full_corpus_deck.py`
...
- [x] 1.2 **Test first (byte-reproducibility):** add
...
- [x] 1.3 **Test first (isolation guard, unit):** add
...
- [x] 1.4 **Test first (isolation guard, CLI wiring):** add
...
- [x] 1.5 **Test first (static path inequality):** add
  ...
  generation invoked.
- [x] 1.6 **Implement** `examples/prelim_sweep_fine/generate_full_corpus.py`: `BASE_INPUTS =
...
- [x] 1.7 **Verify:** `uv run pytest tests/test_full_corpus_deck.py -v`.

---

## 2. Generate and commit the 27-config decks/manifest (cluster-free)

- [x] 2.1 Run `uv run python examples/prelim_sweep_fine/generate_full_corpus.py --timestamp
...
- [x] 2.2 **Verify before committing:** run `git status --porcelain examples/prelim_sweep/
...
- [x] 2.3 Commit `examples/prelim_sweep_fine/inputs/inputs.3d.*` (27 decks),
...
- [x] 3.1 **Test first:** create `tests/test_submit_workflow_parallelism.py`. In a `tmp_path`
...
- [x] 3.2 **Implement**: parse `--parallelism` alongside the existing `--image`/`--namespace`/etc.
...
- [x] 3.3 **Verify:** run the new test suite. Confirm `cluster/argo/scripts/submit_workflow.sh
...
- [x] 3.4 **Update CI lint scope:** add `examples/prelim_sweep_fine/` to both hardcoded path lists
...
- [x] 4.1 Add `openspec/changes/add-fine-grid-corpus-full/specs/force-surrogate/spec.md` with
...
- [x] 4.2 `openspec validate add-fine-grid-corpus-full --strict` passes with zero errors.
- [x] 4.3 **Update `openspec/project.md`**: add a bullet under `### Implemented` noting the
...
- [x] 5.2 `uv run pytest tests/test_full_corpus_deck.py tests/test_submit_workflow_parallelism.py
  -v` — all new tests pass.
- [x] 5.3 Full suite `uv run pytest` is green (no regressions in existing pilot/coarse-corpus
...
- [x] 6.1 Commit grouping, each individually green under **local** verification (`uv run pytest`
...
- [x] 6.2 PR body does not reference any issue with a closing keyword unless actually closing it —
...
- [x] 6.3 Single PR, opened after the full commit sequence is locally green.
