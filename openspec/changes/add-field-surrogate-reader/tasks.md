...
1. [ ] **Test first** (`tests/test_field_surrogate_snapshot.py`): in a **subprocess**
...
2. [ ] **Test first**: a static AST check over `src/mosquito_cfd/field_surrogate/*.py` asserting every
   `import yt` / `from yt import …` node has a function as its nearest enclosing scope. Survives a
   future fourth submodule.
3. [ ] Create `__init__.py` plus `snapshot.py`, `corpus.py`, `domino_adapter.py` as **docstring-only
...
4. [ ] **Test first**: reading the fixture over full extent gives `u`/`v`/`w` matching the analytic
   ...
   to the fixture path.
5. [ ] Implement `FieldSnapshot` (frozen dataclass) and a minimal full-extent `read_field_snapshot`
   with lazy `import yt`.
6. [ ] **Test first**: snapshot arrays report `flags.writeable is False`; two reads of the same region
   do not share memory (`np.shares_memory` is `False`); assignment into a returned array raises.
7. [ ] **Test first**: `fields=("u","density","tracer")` returns exactly those three, `field_names` in
...
8. [ ] **Test first**: an unknown short name, and a canonical name absent from the plotfile, each raise
...
9. [ ] **Test first**: a dataset proxy returning `float32` for one required field and `float64` for
...
10. [ ] Implement the canonical short-name map, field validation, and the dtype-before-cast guard.

## 3. Region semantics (the D1 compatibility surface)

11. [ ] **Test first**: parametrize over the clamping matrix on the 6³ fixture — full `±inf`; a
...
12. [ ] Implement the region semantics, preserving `i_hi = min(max(i_hi, i_lo + 1), ddims)` exactly.
13. [ ] **Test first**: decide and pin the behaviour for a non-finite, non-`±inf` `lo`/`hi` (today
...
14. [ ] **Test first**: `coords (N,3)` / `values (N,F)` with `N == nx·ny·nz`; a chosen cell's row index
...
15. [ ] **Test first**: a 1-cell snapshot flattens to `(1,3)`/`(1,F)` and a 0-cell snapshot to
    `(0,3)`/`(0,F)` — no raise, no collapsed dimension.
16. [ ] Implement `PointCloud` and `FieldSnapshot.to_point_cloud()`.

## 5. AMR refusal

17. [ ] **Test first**: a hand-written proxy wrapping the **real** fixture dataset, overriding only
...
18. [ ] Implement the guard with the CC-F3-naming message. Note in the commit body that the message
...
25. [ ] **Test first** (`tests/test_field_surrogate_corpus.py`): a synthetic corpus tree under
...
26. [ ] **Test first**: `steps()` on a dir containing `plt00100`, `plt01000`, a regular **file** named
...
27. [ ] **Test first**: a missing config, a missing step, and a missing corpus root each raise a
...
28. [ ] Implement `FieldCorpus`, reusing `force_surrogate.dataset.load_manifest_configs` (D8).

## 8. DoMINO volume-half adapter (`domino_adapter.py`)

29. [ ] **Test first** (`tests/test_field_surrogate_domino_adapter.py`): the returned dict's key set is
    **exactly** the five volume-half keys with shapes `(N,3)`, `(N,F)`, `(nx,ny,nz,3)`, `(P,1)`,
    ...
    not zero-filled.
30. [ ] **Test first**: `grid[0,0,0] == (x[0], y[0], z[0])` **and** `grid[nx-1,0,0] == (x[-1], y[0],
    z[0])` — the second is what catches an `indexing="xy"` meshgrid swap, since `[0,0,0]` is invariant
    under it.
31. [ ] **Test first**: (a) an AST check that `domino_adapter.py` contains no `import torch` /
...
32. [ ] **Test first**: the adapter's documentation states that `volume_fields` is a target rather than
...
33. [ ] Implement `to_domino_volume` with the D6 docstring.
...
35. [ ] Document `lev_boxlib_plt/` in `tests/fixtures/README.md` (currently titled "force-surrogate"
...
36. [ ] `openspec/project.md`: add **both** `visualization/` (already missing) and `field_surrogate/`
    to the directory tree, plus a short `field_surrogate` section mirroring "Visualization Tooling".
37. [ ] `openspec/project.md`, four verified accuracy fixes (D9): `line-length: 100 → 88` noting
