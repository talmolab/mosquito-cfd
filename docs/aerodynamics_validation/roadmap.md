...
| **T1b** | **Force-extraction fix** — implement the principled extractor in `mosquito_cfd.benchmarks`, replacing the ad-hoc ~2.4× factor. **First step (per T1a): run the re-run-free CV/surface-stress cross-check — its result picks the path below.** | Sphere **Cd → 1.087 ± tol** (both grids); grid pair *converges toward* literature. | **CV cross-check = No (analysis-only).** If it gives ≈1.087 → done, no re-run. If ≈0.45 → solver fix + re-run (→ deferred per guard, co-land T2a). | **pre-June 30 OK iff analysis-only** |
