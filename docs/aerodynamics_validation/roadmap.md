...
| Axis convention | force **invariance** under the documented transform, graded with the **T1b extractor on both** old- and new-convention runs (never new-extractor-new-geom vs old-extractor-old-geom); **plus the body-frame (chord/normal) per-component van Veen comparison deferred here by #36** | T2a | issue #1 |
...
| Flapping wing — **plausibility gate** | CF_x, CF_z within the literature band **[0.5, 1.5]** **without** the correction factor — **lab-frame magnitude MET in #36** (CF_x 1.41, CF_z 0.68, analysis-only on the committed coarse run); T2b/T3 re-grade the **body-frame** per-component gate on the new-convention re-runs | #36 (lab) → T2b/T3 (body) | van Veen et al. 2022, *JFM* 936:A3 |
...
- **CC-V4. Two distinct defects, kept separate.** T1 changes *how force magnitude is reconstructed*; the
  ...
  changes *orientation/labeling* (forces are already axis-invariant per issue #1 — though #36's #1 note
  flags that the stroke-about-span *motion* may be more than a label, to verify in T2a). Never conflate the
