...
| Flapping wing — **plausibility gate** | CF_x, CF_z within the literature band **[0.5, 1.5]** **without** the correction factor — **lab-frame magnitude MET in #36** (CF_x 1.41, CF_z 0.68, analysis-only on the committed coarse run); the **body-frame** per-component gate was **delivered in T2a** on the new-convention coarse re-run — **CF_normal PASS (2.61 vs ~2.4), CF_chord PARTIAL (0.92 vs ~0.3, #40)**; T3 re-grades on the medium grid | #36 (lab) → **T2a** (body, PARTIAL) → T3 (medium) | van Veen et al. 2022, *JFM* 936:A3 |
...
| **T2b** ⬅️ **NEXT** | **Literature re-validation** — re-run sphere/ellipsoid/flapping (new convention, T1b extractor); grade against oracles; reconcile RESULTS **#3** (CF_x bound) and the **METHODS.md IAMReX commit pin** (skewed: METHODS `c5f8e2a` vs the fork pin now `f93dc794` post-T2a). (#4 CF_z label already fixed + closed in T2a.) | Sphere Cd=1.087; ellipsoid self-consistency + added-mass sanity; flapping CF in band without fudge; RESULTS reproducible. | Yes (coarse) | **in-bounds (post-submission window open)** |
