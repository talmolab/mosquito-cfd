...
| ✅ **T4** ([#40](https://github.com/talmolab/mosquito-cfd/issues/40), PR [#51](https://github.com/talmolab/mosquito-cfd/pull/51), fine-grid repoint after PR [#52](https://github.com/talmolab/mosquito-cfd/pull/52)) | **Per-component force validation + decomposition** — build van Veen's **quasi-steady model (translational + added-mass + Wagner)** from its published coefficients, apply it to our kinematics, and compare each component + the total to the CFD `ib_force` (the model is **replotted, not a digitized Fig 3–4**; Fig 4 = the coefficient-vs-α polars we build it from, Fig 13 = van Veen's time-resolved mosquito curves). Resolves the T2a CF_chord PARTIAL. This tier **validates the wing against van Veen's quasi-steady model** (in peak magnitude — a consistency result at matched kinematics, not an independent measurement of the per-component split); T2b/T3 only establish in-band plausibility. | **DONE.** Now compares against the **fine 256³ grid** (`forces_fine.csv`, T3c) — the finest available, superseding the original coarse-grid comparison. Normal peak **magnitude** graded (model 2.48 vs CFD 2.23, relative gap ~11 % < 16 % tol) + decomposition **closure**; the normal peak-**phase** gap ~0.055 cycle (the CFD *leads* the QS model — expected quasi-steady-vs-unsteady discrepancy) and the curve RMSE are **reported, not gated**; the translational-chord known-answer ~0.42 (the #40 apples-to-oranges vs the total); at the fine grid the CFD chord (0.41) sits close to the model (0.43) — the convergence chain (0.92 coarse → 0.554 medium → 0.41 fine, Richardson 0.32) is reported via the T3c 3-grid analysis (`GCI_fine=27.6 %`), the tight grid verdict deferred to **[#50](https://github.com/talmolab/mosquito-cfd/issues/50)**'s further-refinement follow-up. **#40 resolved.** | reuses T3 runs | **DONE** |
| ✅ **T3c** ([#50](https://github.com/talmolab/mosquito-cfd/issues/50), PR [#52](https://github.com/talmolab/mosquito-cfd/pull/52)) | **Fine-grid 256×128×256 run + 3-grid Richardson study** — ran on local RTX A5000 (dt=0.00025 D6 fallback, 4000 steps, ~8.8 h); `iamrex_commit=f93dc794`. 3-grid Richardson: **CF_normal monotone** (p_obs=1.38, Richardson=2.162, GCI_fine=3.7%); **CF_chord monotone** (0.923→0.554→0.411, p_obs=1.37, Richardson=0.321, GCI_fine=27.6%; large GCI — chord not yet tightly converged at 256³). Further refinement (512³+) deferred to H100/grant. | **DONE.** `forces_fine.csv` + `run_metadata_t3c.json` committed; RESULTS.md T3c section + reproducibility guard added; #50 closed. | Yes (local A5000) | **DONE** |

**Sequencing.** T1a → T1b → T2a → T2b → T3a → T3b → T4 → **T3c**. **T1a…T3c complete (2026-07).**
...
- **Fine grid (512³+) flapping beyond T3c** — T3c (256³, #50) is **complete**. CF_chord GCI_fine=27.6% shows the chord is not yet tightly converged at 256³; further refinement (512³+) defers to H100 (grant).
