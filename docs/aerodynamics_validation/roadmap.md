...
| 🟡 **T3a** ([#46](https://github.com/talmolab/mosquito-cfd/issues/46)) | **Medium-grid convergence tooling + deck (cluster-free)** — re-author the new-convention 128×64×128 deck (`inputs.3d.convergence_medium`; **only `amr.n_cell` changes** vs the confirmed coarse `inputs.3d.validation`, the old `inputs.3d.production` is old-BC/different-operating-point/already-128³ and is NOT reused), and build + TDD a **report-only** 2-grid convergence grader (`benchmarks/wing_convergence.py`) + LEV vorticity/Q pure functions (`benchmarks/lev.py`) against the committed coarse run + synthetic fixtures. **Quantify** grid sensitivity; do not gate it. | **Tooling delivered + unit-tested cluster-free** — deck-invariance (differs from coarse **only** in `amr.n_cell`; `fixed_dt`/`radius` held), grader known-answer + report-only + degenerate/sign guards, LEV known-analytic vorticity (`\|ω\|=2Ω`, `Q=Ω²`). **No graded run here** (grading is T3b). | No (analysis-only) | **in-bounds** |
| ⬜ **T3b** ([#46](https://github.com/talmolab/mosquito-cfd/issues/46)) | **Medium-grid run + grading (operator A40)** — run the medium sim (`:fp64 @ f93dc794`, `capture_run_metadata`), commit `forces_medium.csv` + run metadata, apply the T3a grader + LEV to the real data, and write the RESULTS convergence section (handoff: `docs/aerodynamics_validation/t3b-handoff.md`). | **Report** the coarse↔medium **relative change + 2-grid GCI band (p = 1..2)** on peak body-frame CF_chord/CF_normal — **reported, no pass bar, no loosenable tolerance**; LEV vorticity/Q **reported, not gated**. "Not converged at coarse" is a valid, informative outcome (feeds #40). The true observed-order (3-grid/256³) verdict is deferred (H100/grant). | Yes (A40 unattended) | **in-bounds** |
...
**Sequencing.** T1a → T1b → T2a → T2b → T3a → T3b → T4. **T1a/T1b/T2a/T2b complete (2026-07); T3a
(tooling+deck) is in flight, T3b (operator run) follows.** T1 was the only pre-deadline work; with June
30 passed, T3–T4 are in-bounds (post-submission window).
