...
**Sequencing.** T1a → T1b → T2a → T2b → T3a → T3b → T4. **T1a…T4 complete (2026-07).** T4
(per-component force decomposition, #40) **validated the wing against van Veen's quasi-steady model** (in
...
The tight chord *grid* verdict (256³) remains the deferred T3c line
([#50](https://github.com/talmolab/mosquito-cfd/issues/50)). T3b landed the graded medium run + the RESULTS
"Grid convergence (T3b)" section (CF_chord drops 66.5 % under refinement — supports coarse-grid
diffused-IB for #40, not yet grid-converged). T1 was the only pre-deadline work; with June 30 passed,
T4 was in-bounds (post-submission window).
...
- **Fine grid (256³+) flapping** ([#50](https://github.com/talmolab/mosquito-cfd/issues/50)) — expensive;
  defer to H100 (grant) unless the coarse/medium pair already satisfies the T3/T4 oracles.
