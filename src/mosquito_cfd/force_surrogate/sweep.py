...
snapshot of the validated deck (``examples/prelim_sweep/base_inputs.3d.validation``) as the base.
NB Tier T2a re-oriented the live ``examples/flapping_wing/inputs.3d.validation`` to the van Veen
convention; the corpus templates off a decoupled snapshot rather than the live deck so routine
convention edits don't force a corpus regeneration (CC-V6). That decoupling itself let a
geometry defect slip through once (the snapshot's hinge was correct only under the pre-refactor
convention, once wing.vertex moved to the new one) -- fixed by ``fix-force-surrogate-sweep-hinge``,
which corrected the hinge and regenerated the corpus as a documented, one-time exception; the
snapshot remains decoupled from the live deck's future edits. Pure and cluster-free: no RunAI,
GPU, or plotfiles (roadmap CC-2, CC-6).
