...
| 🟡 **T2b** | **Literature re-validation** — grade sphere/ellipsoid/flapping against oracles (T1b extractor, van Veen convention); reconcile RESULTS **#3** (reproducibility) and the **METHODS.md IAMReX pin** (repo+hash: upstream `c5f8e2a` → fork `talmolab/IAMReX @ f93dc794`). (#4 CF_z label already fixed + closed in T2a.) | **Sphere = H1′** (Richardson 1.131 ÷ +3–6% confinement → `[1.067,1.098]` ∋ 1.087 ±5%, no re-run); **flapping = band-as-floor** (peaks clear the `0.5` O(1) floor; CF_x 2.37 > ceiling expected; body-frame PARTIAL carried, #40) **+ RESULTS #3 reproducible + closed**; **METHODS pin reconciled** (prefix-match guard). **Ellipsoid self-consistency + added-mass sanity = pending operator re-run** (grader + synthetic-fixture numerics landed; the A40 re-run for the `SumU` 29-col CSV is the one cluster step). | Yes (ellipsoid coarse) | **in-flight (cluster-free workstreams done; ellipsoid re-run pending)** |
