...
| ✅ **T3b** ([#46](https://github.com/talmolab/mosquito-cfd/issues/46)) | **Medium-grid run + grading (operator A40)** — ran the medium sim (`:fp64 @ f93dc794`, A40, `capture_run_metadata`), committed `forces_medium.csv` + `run_metadata_t3b.json`, applied the T3a grader + LEV to the real data, wrote the RESULTS convergence section. | **DONE.** Reported the coarse↔medium **relative change + 2-grid GCI band (p = 1..2)**: peak **CF_chord −66.5 %** (0.923→0.554, GCI band 0.28–0.83) — a large drop **supporting the coarse-grid diffused-IB hypothesis** for the #40 chord excess, though **not grid-converged at medium** (chord `gci_p1=0.83`, medium 0.554 still ~1.8× van Veen); **CF_normal −11.7 %** (grid-settled). LEV present on **both** grids at mid-stroke (resolution-fair ∫Q⁺ +9 %; peak Q ×3 is a resolution artifact). Report-only, no pass bar. True observed-order (3-grid/256³) verdict deferred (H100/grant); **#40 advanced, not resolved** (T4 reuses these runs). | Yes (A40 unattended) | **in-bounds** |
