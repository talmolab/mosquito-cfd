...
- **Digest required (content-addressable):** the base stores the image as a flat `docker_image` string and silently omits it when falsy. The wrapper strips surrounding whitespace and **raises `ValueError` unless the value contains `sha256:`** — so a blank/missing value *or a mutable tag like `:latest`* is rejected and no surrogate run is recorded against a non-reproducible image. (Tests assert `meta["docker_image"] == digest`, that a tag raises, and that whitespace is stripped.)
